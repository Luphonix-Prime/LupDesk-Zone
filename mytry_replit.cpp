...
class AudioCapture {
public:
    AudioCapture() : m_device(nullptr), m_client(nullptr), m_capture(nullptr), m_running(false) {}
    ~AudioCapture() { stop(); }
        ...
        while (m_running) {
            ...
            while (packetLength != 0 && m_running) {
                ...
                if (!(flags & AUDCLNT_BUFFERFLAGS_SILENT) && numFramesAvailable > 0) {
                    uint32_t size = numFramesAvailable * 4; // Assuming 16-bit stereo
                    if (sendAll(m_audioSocket, (char*)&size, 4) == SOCKET_ERROR) break;
                    if (sendAll(m_audioSocket, (char*)pData, size) == SOCKET_ERROR) break;
                }
...
private:
    ...
    void playbackLoop() {
        while (m_running) {
            ...
            if (r != 4 || size == 0 || size > 1000000) break;
            ...
            UINT32 numFramesToWrite = size / 4; // Assuming 16-bit stereo
