...
private:
    ...
    void captureLoop() {
        ...
        HRESULT hr = m_client->GetMixFormat(&pwfx);
        if (FAILED(hr)) {
            std::cerr << "Audio capture GetMixFormat failed\n";
            return;
        }
        
        UINT32 bytesPerFrame = pwfx ? pwfx->nBlockAlign : 4;
        
        // Calculate the actual buffer duration
        REFERENCE_TIME hnsRequestedDuration = 10000000; // 1 second
        REFERENCE_TIME hnsActualDuration;
        UINT32 bufferFrameCount;
        hr = m_client->GetBufferSize(&bufferFrameCount);
        if (SUCCEEDED(hr)) {
            hnsActualDuration = (REFERENCE_TIME)((10000.0 * 1000 * bufferFrameCount) / pwfx->nSamplesPerSec);
        } else {
            hnsActualDuration = hnsRequestedDuration;
        }
        ...
        while (m_running) {
            ...
            // Wait for the next buffer event or sleep for a small amount
            Sleep(10);
        }
...
class AudioPlayback {
public:
    AudioPlayback() : m_enumerator(nullptr), m_device(nullptr), m_client(nullptr), m_render(nullptr), m_running(false) {}
    ~AudioPlayback() { stop(); }
    ...
    bool start(SOCKET audioSocket) {
        ...
        HRESULT hr = CoCreateInstance(__uuidof(MMDeviceEnumerator), NULL, CLSCTX_ALL,
            __uuidof(IMMDeviceEnumerator), (void**)&m_enumerator);
        if (FAILED(hr)) {
            std::cerr << "AudioPlayback: Failed to create device enumerator\n";
            return false;
        }

        hr = m_enumerator->GetDefaultAudioEndpoint(eRender, eConsole, &m_device);
        if (FAILED(hr)) {
            std::cerr << "AudioPlayback: Failed to get default audio endpoint\n";
            return false;
        }

        hr = m_device->Activate(__uuidof(IAudioClient), CLSCTX_ALL, NULL, (void**)&m_client);
        if (FAILED(hr)) {
            std::cerr << "AudioPlayback: Failed to activate audio client\n";
            return false;
        }
        ...
        hr = m_client->GetMixFormat(&pwfx);
        if (FAILED(hr)) {
            std::cerr << "AudioPlayback: Failed to get mix format\n";
            return false;
        }

        hr = m_client->Initialize(AUDCLNT_SHAREMODE_SHARED, 0, 10000000, 0, pwfx, NULL);
        if (FAILED(hr)) {
            std::cerr << "AudioPlayback: Failed to initialize audio client\n";
            CoTaskMemFree(pwfx);
            return false;
        }
        ...
        CoTaskMemFree(pwfx);
        if (FAILED(hr)) {
            std::cerr << "AudioPlayback: Failed to get render service\n";
            return false;
        }

        hr = m_client->Start();
        if (FAILED(hr)) {
            std::cerr << "AudioPlayback: Failed to start audio client\n";
            return false;
        }
...
private:
    ...
    void playbackLoop() {
        ...
        HRESULT hr = m_client->GetMixFormat(&pwfx);
        if (FAILED(hr)) {
            std::cerr << "AudioPlayback: Failed to get mix format in playback loop\n";
            return;
        }
        ...
        while (m_running) {
            ...
            if (FAILED(hr)) {
                std::cerr << "AudioPlayback: Failed to get current padding\n";
                continue;
            }
            ...
            hr = m_client->GetBufferSize(&bufferFrameCount);
            if (FAILED(hr)) {
                std::cerr << "AudioPlayback: Failed to get buffer size\n";
                continue;
            }
...
public:
    ...
    bool create(int w, int h, void* client = nullptr) {
        ...
        wc.hbrBackground = (HBRUSH)(COLOR_WINDOW+1); // Add background color
    ...
    void updateTile(int x, int y, HBITMAP tile) {
        ...
        BITMAP tb; 
        if (!GetObject(tile, sizeof(tb), &tb)) return;
...
private:
    ...
    static LRESULT CALLBACK WndProcStatic(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
        ...
        if (msg == WM_PAINT) {
            ...
            int serverX = (mouseX * self->width) / max(1, rc.right);
            int serverY = (mouseY * self->height) / max(1, rc.bottom);
